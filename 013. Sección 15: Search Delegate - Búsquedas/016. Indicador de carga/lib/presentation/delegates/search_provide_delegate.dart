...
//Paso 3: Lógica final y optimización
...
class SearchMovieDelegate extends SearchDelegate<Movie?> {
  ...
  List<Widget>? buildActions(BuildContext context) {
    return [
      // Paso 3.1: StreamBuilder para escuchar el estado
      StreamBuilder(
        initialData: false,// Asumimos que al inicio NO está cargando.
        stream: isLoadingStream.stream,
        builder: (context, snapshot) {

          // Si snapshot.data es true (está cargando
          if (snapshot.data ?? false) {
            return SpinPerfect(
              duration: const Duration(seconds: 20), // Giro lento y elegante
              spins: 10,
              infinite: true, // Nunca para hasta que se desmonte
              child: IconButton(
                onPressed: () => query = '', // Permitimos cancelar/borrar incluso cargando
                icon: const Icon(Icons.refresh_rounded), // Icono de refresco visual
              ),
            );
          }
          // Paso 3.2: Estado de Reposo (Botón Limpiar)
          // Si no está cargando, mostramos la X con animación de entrada.
          FadeIn(
            animate: query.isNotEmpty,// Solo aparece si hay texto
            duration: const Duration(milliseconds: 200),
            child: IconButton(
              onPressed: () => query = '',
              icon: const Icon(Icons.clear),
